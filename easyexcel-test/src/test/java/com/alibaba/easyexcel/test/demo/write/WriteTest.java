...
public class WriteTest {
    ...
    public void simpleWrite() {
        ...
        CustomHorizontalCellStyleStrategy horizontalCellStyleStrategy =
                new CustomHorizontalCellStyleStrategy(headWriteCellStyle, contentWriteCellStyle);

        // 注意 simpleWrite在数据量不大的情况下可以使用（5000以内，具体也要看实际情况），数据量大参照 重复多次写入

        // 写法1 JDK8+
        // since: 3.0.0-beta1
        String fileName = "D:\\github\\easyexcel\\easyexcel-test\\src\\test\\java\\com\\alibaba\\easyexcel\\test\\demo\\write\\simpleWrite123.xlsx";
...
//        ExcelWriterBuilder excelWriterBuilder = EasyExcel.write(fileName);
        ...
        List<String> doubleData = new ArrayList<>();
        string.add("字符串1");
        date.add("date");
        doubleData.add("doubleData");
        head.add(string);
        head.add(date);
        head.add(doubleData);
//        excelWriterBuilder.registerWriteHandler(new CommentWriteHandler()).registerWriteHandler(horizontalCellStyleStrategy);
//        excelWriterBuilder.head(head);
//        excelWriterBuilder.sheet("模板")
//                .doWrite(()-> mapData());
...
//        }
    ...
    private List<Map<Integer, Object>> mapData() {
    ...
    private List<Object> mapDataObj() {
        List<Object> list = ListUtils.newArrayList();
        for (int i = 0; i < 10; i++) {
            Map<String, Object> data = new LinkedCaseInsensitiveMap<>();
            data.put("string", "字符串" + i);
            data.put("date", new Date());
            data.put("doubleData", 0.56);
            list.add( PropertyUtils.generate( DemoData.class, data));
        }
        return list;
    }


    private Map<String, Object> mapData(Integer i) {
